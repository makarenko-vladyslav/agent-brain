...
description: Для роботи з Gemini API, @google/genai SDK, Gemini моделями. НЕ для Claude/OpenAI API.
metadata: {"tags": ["gemini", "google", "genai", "multimodal", "function-calling", "structured-output"]}
---

# Gemini API Development Skill

### TRIGGERS
- Ключові слова: "Gemini", "@google/genai", "google-genai", "Gemini API", "Gemini Flash", "Gemini Pro"
- Код: `import { GoogleGenAI }`, `from google import genai`, `google.golang.org/genai`
- Задачі: інтеграція Gemini, multimodal з Gemini, function calling з Gemini, structured output з Gemini

### ANTI-TRIGGERS
- Claude/Anthropic API → `applied-ai-automation` або skill `claude-api`
- OpenAI API → `applied-ai-automation`
- Загальна AI архітектура (RAG, agents) → `applied-ai-automation`
- Конфігурація моделей в проєкті → brain/rules-archive/ai-prompts.md

### SKILL CONFLICTS
- `applied-ai-automation` — genai-sdk = тільки Gemini SDK, applied-ai = загальна AI архітектура
- Якщо проєкт використовує Vercel AI SDK з Gemini → `applied-ai-automation` (Vercel обгортка)
